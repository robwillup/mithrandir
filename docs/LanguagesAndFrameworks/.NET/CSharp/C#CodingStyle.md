...
Coding conventions server the following purposes:
...
to be visible to external callers.
...
2. We always specify the visibility, even if it's the default (e.g. `private string _foo` not
...
1.  We use language keywords instead of BCL types (e.g. `int, string, float` instead of
...
2.  Fields should be specified at the top within type declarations.

3.  When including non-ASCII characters in the source code use Unicode escape sequences (\uXXXX)
...
2.  When using a single-statement if, we follow these conventions:
...
3.  Make all internal and private types static or sealed unless derivation from them is required.
